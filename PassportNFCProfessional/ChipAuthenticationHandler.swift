...
class ChipAuthenticationHandler {
    ...
    public init(dg14 : DataGroup14, pc: PassportController) {
        self.pc = pc
    ...
    private func sendPublicKey(oid : String, keyId : Int?, pcdPublicKey : OpaquePointer) async throws {
        ...
        if cipherAlg.hasPrefix("DESede") {
            ...
            _ = try await self.pc?.sendMSEKAT(keyData: Data(wrappedKeyData), idData: Data(idData))
        } else if cipherAlg.hasPrefix("AES") {
            _ = try await self.pc?.sendMSESetATIntAuth(oid: oid, keyId: keyId)
    ...
    private func handleGeneralAuthentication() async throws {
        repeat {
            ...
            // send it
            _ = try await self.pc?.sendGeneralAuthenticate(data: segment, isLast: isLast)
