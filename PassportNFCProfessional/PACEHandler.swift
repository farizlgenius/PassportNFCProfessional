...
//    var tagReader : TagReader
    ...
    public init(cardAccess : CardAccess, pc: PassportController) throws {
        self.pc = pc
    ...
    public func doPACE( mrzKey : String ) async throws {
        ...
        _ = try await pc.sendMSESetATMutualAuth(oid: paceOID, keyType: paceKeyType)
...
*/
    ...
    func doStep1() async throws -> [UInt8] {
        ...
        let response = try await pc.sendGeneralAuthenticate(data: [], isLast: false)
    ...
    func doPACEStep2GM(passportNonce : [UInt8]) async throws -> OpaquePointer {
        ...
        let response = try await pc.sendGeneralAuthenticate(data:step2Data, isLast:false)
...
///         - Tuple of Generated Ephemeral KeyPair and the Passport's public key
    func doStep3KeyExchange(ephemeralParams: OpaquePointer) async throws -> (OpaquePointer, OpaquePointer) {
        ...
        let response = try await pc.sendGeneralAuthenticate(data:step3Data, isLast:false)
    ...
    func doStep4KeyAgreement( pcdKeyPair: OpaquePointer, passportPublicKey: OpaquePointer) async throws -> ([UInt8], [UInt8]) {
        ...
        let response = try await pc.sendGeneralAuthenticate(data:step4Data, isLast:true)
