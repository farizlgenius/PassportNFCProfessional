...
public func desMAC(key : [UInt8], msg : [UInt8]) -> [UInt8]{
    ...
    AppLogger.passportReader.debug("Calc mac" )
    for i in 0 ..< size {
        let tmp = [UInt8](msg[i*8 ..< i*8+8])
        AppLogger.passportReader.debug("x\(i): \(binToHexRep(tmp))" )
        y = DESEncrypt(key: [UInt8](key[0..<8]), message: tmp, iv: y)
        AppLogger.passportReader.debug("y\(i): \(binToHexRep(y))" )
    }
    
    AppLogger.passportReader.debug("y: \(binToHexRep(y))" )
    AppLogger.passportReader.debug("bkey: \(binToHexRep([UInt8](key[8..<16])))" )
    AppLogger.passportReader.debug("akey: \(binToHexRep([UInt8](key[0..<8])))" )
    let iv : [UInt8] = [0,0,0,0,0,0,0,0]
    let b = DESDecrypt(key: [UInt8](key[8..<16]), message: y, iv: iv, options:UInt32(kCCOptionECBMode))
    AppLogger.passportReader.debug( "b: \(binToHexRep(b))" )
    let a = DESEncrypt(key: [UInt8](key[0..<8]), message: b, iv: iv, options:UInt32(kCCOptionECBMode))
    AppLogger.passportReader.debug( "a: \(binToHexRep(a))" )
