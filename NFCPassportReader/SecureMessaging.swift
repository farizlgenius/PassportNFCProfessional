...
public class SecureMessaging {
    ...
    func protect(apdu : NFCISO7816APDU, useExtendedMode: Bool = false ) throws -> NFCISO7816APDU {
        ...
        AppLogger.secureMessaging.debug("\t\tSSC: \(binToHexRep(self.ssc))")
        self.ssc = self.incSSC()
        let paddedSSC = algoName == .DES ? self.ssc : [UInt8](repeating: 0, count: 8) + ssc
        AppLogger.secureMessaging.debug("\tIncrement SSC with 1")
        AppLogger.secureMessaging.debug("\t\tSSC: \(binToHexRep(self.ssc))")
        ...
        AppLogger.secureMessaging.debug("\(tmp)")
        AppLogger.secureMessaging.debug("\tM: \(binToHexRep(M))")
        
        AppLogger.secureMessaging.debug("Compute MAC of M")
        
        let N = pad(paddedSSC + M, blockSize:padLength)
        AppLogger.secureMessaging.debug("\tConcatenate SSC and M and add padding")
        AppLogger.secureMessaging.debug("\t\tN: \(binToHexRep(N))")

        var CC = mac(algoName: algoName, key: self.ksmac, msg: N)
        if CC.count > 8 {
            CC = [UInt8](CC[0..<8])
        }
        AppLogger.secureMessaging.debug("\tCompute MAC over N with KSmac")
        AppLogger.secureMessaging.debug("\t\tCC: \(binToHexRep(CC))")
        ...
        AppLogger.secureMessaging.debug("Construct and send protected APDU")
        AppLogger.secureMessaging.debug("\tProtectedAPDU: \(binToHexRep(protectedAPDU))")
...
//    /// Protect APDU according to ICAO Doc 9303 secure messaging.
//    /// Refactored to correctly support:
//    /// - AES PACE secure messaging
//    /// - short / extended length APDU
//    /// - proper DO97 handling
//    /// - correct SSC usage
//    /// Protect APDU according to ICAO 9303 secure messaging
//    func protect(
//        apdu: NFCISO7816APDU,
...
//            algoName == .DES
//            ? self.ssc
//            : [UInt8](repeating: 0, count: 8) + self.ssc
//
//        AppLogger.secureMessaging.debug("\tIncrement SSC with 1")
//        AppLogger.secureMessaging.debug("\t\tSSC: \(binToHexRep(self.ssc))")
//
...
//        let M = cmdHeader + do87 + do97
//
//        AppLogger.secureMessaging.debug("Concatenate CmdHeader + DO87 + DO97")
//        AppLogger.secureMessaging.debug("\tM: \(binToHexRep(M))")
//
//        let N = pad(paddedSSC + M, blockSize: padLength)
//
...
//
//        if CC.count > 8 {
//            CC = Array(CC.prefix(8))
//        }
//
//        let do8e = self.buildD08E(mac: CC)
//
//        let protectedData = do87 + do97 + do8e
//
//        var protectedAPDU: [UInt8] = []
//
//        // CLA INS P1 P2
//        protectedAPDU += Array(cmdHeader.prefix(4))
//
//        // Lc
//        if protectedData.count > 255 ||
//            (useExtendedMode && apdu.expectedResponseLength > 231) {
//
//            protectedAPDU += [0x00]
//            protectedAPDU += intToBin(protectedData.count, pad: 2)
//
//        } else {
//
//            protectedAPDU += [UInt8(protectedData.count)]
//        }
//
//        // Data
//        protectedAPDU += protectedData
//
//        // ALWAYS append Le for SM
//        if protectedData.count > 255 ||
//            (useExtendedMode && apdu.expectedResponseLength > 231) {
//
//            protectedAPDU += [0x00, 0x00]
...
//
//        AppLogger.secureMessaging.debug("Construct and send protected APDU")
//        AppLogger.secureMessaging.debug(
//            "\tProtectedAPDU: \(binToHexRep(protectedAPDU))"
//        )
//
//        let newAPDU = NFCISO7816APDU(data: Data(protectedAPDU))!
//
...
//    }
    ...
    func unprotect(rapdu : ResponseAPDU ) throws -> ResponseAPDU {
        ...
        AppLogger.secureMessaging.debug("Receive response APDU of MRTD's chip")
        AppLogger.secureMessaging.debug("\tRAPDU: \(binToHexRep(rapduBin))")
        ...
        guard rapduBin.count >= offset + 5 else {
            AppLogger.secureMessaging.error("size error")
        ...
        if rapduBin[offset] == 0x8E {
            ...
            AppLogger.secureMessaging.debug("Verify RAPDU CC by computing MAC of \(tmp)")
            
            let K = pad(paddedSSC + do87 + do99, blockSize:padLength)
            AppLogger.secureMessaging.debug("\tConcatenate SSC and \(tmp) and add padding")
            AppLogger.secureMessaging.debug("\t\tK: \(binToHexRep(K))")
            
            AppLogger.secureMessaging.debug("\tCompute MAC with KSmac")
            var CCb = mac(algoName: algoName, key: self.ksmac, msg: K)
            if CCb.count > 8 {
                CCb = [UInt8](CC[0..<8])
            }
            AppLogger.secureMessaging.debug("\t\tCC: \(binToHexRep(CCb))")
            
            let res = (CC == CCb)
            AppLogger.secureMessaging.debug("\tCompare CC with data of DO'8E of RAPDU")
            AppLogger.secureMessaging.debug("\t\t\(binToHexRep(CC))  == \(binToHexRep(CCb)) ? \(res)")
        ...
        if do87Data.count > 0 {
            ...
            // There is a payload
            data = unpad(dec)
            AppLogger.secureMessaging.debug("Decrypt data of DO'87 with KSenc")
            AppLogger.secureMessaging.debug("\tDecryptedData: \(binToHexRep(data))")
        }
        
        AppLogger.secureMessaging.debug("Unprotected APDU: [\(binToHexRep(data))] \(binToHexRep(sw1)) \(binToHexRep(sw2))" )
        return ResponseAPDU(data: data, sw1: sw1, sw2: sw2)
    }

    func maskClassAndPad(apdu : NFCISO7816APDU ) -> [UInt8] {
        AppLogger.secureMessaging.debug("Mask class byte and pad command header")
        let res = pad([0x0c, apdu.instructionCode, apdu.p1Parameter, apdu.p2Parameter], blockSize: padLength)
        AppLogger.secureMessaging.debug("\tCmdHeader: \(binToHexRep(res))")
        return res
    }
    ...
    func buildD087(apdu : NFCISO7816APDU) throws -> [UInt8] {
        ...
        AppLogger.secureMessaging.debug("Build DO'87")
        AppLogger.secureMessaging.debug("\tDO87: \(binToHexRep(res))")
        return res
    }
    ...
    func padAndEncryptData(_ apdu : NFCISO7816APDU) -> [UInt8] {
        ...
        AppLogger.secureMessaging.debug("Pad data")
        AppLogger.secureMessaging.debug("\tData: \(binToHexRep(paddedData))")
        AppLogger.secureMessaging.debug("Encrypt data with KSenc")
        AppLogger.secureMessaging.debug("\tEncryptedData: \(binToHexRep(enc))")
        return enc
    }
    ...
    func buildD08E(mac : [UInt8]) -> [UInt8] {
        ...
        AppLogger.secureMessaging.debug("Build DO'8E")
        AppLogger.secureMessaging.debug("\tDO8E: \(binToHexRep(res))" )
        return res
    }
    ...
    func buildD097(apdu : NFCISO7816APDU) throws -> [UInt8] {
        ...
        AppLogger.secureMessaging.debug("Build DO'97")
        AppLogger.secureMessaging.debug("\tDO97: \(res)")
        return res
    }
