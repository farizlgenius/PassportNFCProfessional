...
//    var tag : NFCISO7816Tag // ===> Default
    var tag : APDUTransceiver
...
//    init( tag: NFCISO7816Tag ) { // ==> Default
//        self.tag = tag
//    }
    
    init( tag: APDUTransceiver ) {
        self.tag = tag
    }
    ...
    func selectFileAndRead( tag: [UInt8]) async throws -> [UInt8] {
        ...
        AppLogger.tagReader.debug( "TagReader - Number of data bytes to read - \(remaining)" )
        ...
        while remaining > 0 {
            ...
            AppLogger.tagReader.debug( "TagReader - data bytes remaining: \(remaining), will read : \(readAmount)" )
            let cmd = NFCISO7816APDU(
                instructionClass: 00,
            ...
            AppLogger.tagReader.debug( "TagReader - got resp - \(binToHexRep(resp.data, asArray: true)), sw1 : \(resp.sw1), sw2 : \(resp.sw2)" )
            data += resp.data
            
            remaining -= resp.data.count
            amountRead += resp.data.count
            AppLogger.tagReader.debug( "TagReader - Amount of data left to read - \(remaining)" )
        }
    ...
    func selectPassportApplication() async throws -> ResponseAPDU {
        ...
        AppLogger.tagReader.debug( "Re-selecting eMRTD Application" )
    ...
    func send( cmd: NFCISO7816APDU, useExtendedMode : Bool = false ) async throws -> ResponseAPDU {
        AppLogger.tagReader.debug( "TagReader - sending \(cmd)" )
        var toSend = cmd
        if let sm = secureMessaging {
            toSend = try sm.protect(apdu:cmd, useExtendedMode: useExtendedMode)
            AppLogger.tagReader.debug("TagReader - [SM] \(toSend)" )
        }
        
        var (data, sw1, sw2) = try await tag.sendCommand(apdu: toSend)
        AppLogger.tagReader.debug( "TagReader - Received response, size \(data.count)b" )
        ...
        while (sw1 == 0x61) {
            ...
            AppLogger.tagReader.debug("Read remaining data. Accumulated: \(data.count + nextSegment.count)b. Last batch \(nextSegment.count)b. Still remaining: \(sw2)b")
            data += nextSegment
        }
        ...
        if let sm = self.secureMessaging {
            ...
            AppLogger.tagReader.debug("\(String(format:"TagReader [SM - unprotected] \(binToHexRep(rep.data, asArray:true)), sw1:0x%02x sw2:0x%02x", rep.sw1, rep.sw2))" )
        } else {
            AppLogger.tagReader.debug("\(String(format:"TagReader [unprotected] \(binToHexRep(rep.data, asArray:true)), sw1:0x%02x sw2:0x%02x", rep.sw1, rep.sw2))" )
            
        }
        
        if rep.sw1 != 0x90 && rep.sw2 != 0x00 {
            AppLogger.tagReader.error( "Error reading tag: sw1 - 0x\(binToHexRep(sw1)), sw2 - 0x\(binToHexRep(sw2))" )
            ...
            if (rep.sw1 == 0x63 && rep.sw2 == 0x00) {
                ...
                AppLogger.tagReader.error( "reason: \(errorMsg)" )
